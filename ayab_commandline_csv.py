...
def mainFunction(options):
    ...
    #instead of loading an image, load in a csv file of 1 and 0
    if options.csv:
      print("looking for CSV file",options.csv)

      fn = options.csv
      with open(fn) as csv_file:
         tmp_csv = csv.reader(csv_file, delimiter=',')
         tmp_list = list(tmp_csv)
         new_arr = []
         ...
         i = Image.fromarray(numpy_csv,mode='L')

         # we save it and reload it by filename
         ff = 'ayab.png'
         i.save(ff)
         image = ayab_image.ayabImage(ff, \
                                  options.num_colors)
         

    if options.image:  
      image = ayab_image.ayabImage(options.image, \
                                  options.num_colors)
...
if __name__ == '__main__':
    ...
    parser = OptionParser("%prog [filename] [options]", \
        description = "AYAB Control Commandline Version with addad csv")
    ...
    parser.add_option("-x", "--csv", \
        dest    = "csv", \
        type    = "string", \
        help    = "Use a csv file not an image [default: %default]")
    parser.add_option("-i", "--image", \
        dest    = "image", \
        type    = "string", \
        help    = "Use an image not a csv file[default: %default]")
    ...
    if(options.image or options.csv):
